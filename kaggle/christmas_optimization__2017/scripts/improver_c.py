...
double get_score_from_preds(double *score, int *tch, int *tsh) {
	...
	for (i = 0; i < 1000000; i++) {
		...
		total_santa_happiness += santa_sc ore[i][preds[i]];
	}
...
if __name__ == '__main__':
    initial_sub = 'subm_0.935841697655.csv'
    shutil.copy(initial_sub, 'subm.csv')
    # os.system('gcc improver.c -o improver')
    os.system('gcc -fopenmp improver.c -o improver')
    for i in range(10):
        print("*"*10 + ' ' + str(i+1) + ' ' + "*"*10)
        os.system('improver.exe')
